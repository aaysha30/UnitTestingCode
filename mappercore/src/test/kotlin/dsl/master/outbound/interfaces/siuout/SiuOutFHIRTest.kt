    ...
    class SiuOutFHIRTest {
        ...
        fun setup() {
            ...
            parameters[ParameterConstant.BUNDLE_UTILITY] = fhirFactory.getBundleUtility()     //.getParameters(a as Bundle) as String
        ...
        @Test
        public fun bundleSizeTest(){

            var appBundle:Bundle=parser.parseResource(Bundle::class.java,bundleString)
            var foundAppBundle=    appBundle.entry.find { it.resource.fhirType()==Enumerations.FHIRAllTypes.APPOINTMENT.toCode() }
            ...
            var patientBundle=appBundle.entry.find {  it.resource.fhirType()==Enumerations.FHIRAllTypes.PATIENT.toCode()  }
           ...
           `when`(fhirClient.search(eq("Patient"), eq("_id"),eq("Patient-944203"))).thenReturn(patientBundl)
            var orgResource=appBundle.entry.find { it.resource.fhirType()==Enumerations.FHIRAllTypes.ORGANIZATION.toCode() }
            var org=    orgResource?.resource as Organization
            `when`(fhirClient.readById("Organization", "Organization-Dept-10011")).thenReturn(org)
            var healthcareservice=appBundle.entry.find { it.resource.fhirType()==Enumerations.FHIRAllTypes.HEALTHCARESERVICE.toCode() }
            ...
            var location1217=appBundle.entry.find { it.resource.fhirType()==Enumerations.FHIRAllTypes.LOCATION.toCode() }
            ...
            var location2796=appBundle.entry.find { it.resource.fhirType()==Enumerations.FHIRAllTypes.LOCATION.toCode() }
            ...
            var location4365=appBundle.entry.find { it.resource.fhirType()==Enumerations.FHIRAllTypes.LOCATION.toCode() }
            ...
            var careTeam944203=appBundle.entry.find { it.resource.fhirType()==Enumerations.FHIRAllTypes.CARETEAM.toCode() }
            ...
            `when`(fhirClient.search(eq("CareTeam"), eq("patient"),eq("Patient-944203"),eq("_include"),eq("*"))).thenReturn(careTeamBundle944203)
            var actDefBundle=   appBundle.entry.find { it.resource.fhirType()==Enumerations.FHIRAllTypes.ACTIVITYDEFINITION.toCode() }
            var actDef= actDefBundle?.resource as ActivityDefinition
            var actDefBundl=Bundle()
            actDefBundl.addEntry().resource=actDef
            `when`(fhirClient.search(eq("ActivityDefinition"), eq("name"),eq("Blood Test"),eq("context-reference"),eq("Organization-Dept-10011"))).thenReturn(actDefBundl)
